...
enum States {initialState, wait, buttonWait, C, D, E, F, G, A, B, C2} state;
...
void tickFct() {
	...
	switch(state) {
		...
		case wait:
			...
			if((button == 0x02)) { //if on, increment scaleCounter when 0x02
				if(scaleCounter < 8) {
					scaleCounter++;
				}
			}
			else if((button == 0x04)) { //if on, decrement scaleCounter when 0x04
			...
			else if((button == 0x04) && (scaleCounter == 2)) {
				state = D;
			}
			else if((button == 0x04) && (scaleCounter == 3)) {
				state = E;
			}
			else if((button == 0x04) && (scaleCounter == 4)) {
				state = F;
			}
			else if((button == 0x04) && (scaleCounter == 5)) {
				state = G;
			}
			else if((button == 0x04) && (scaleCounter == 6)) {
				state = A;
			}
			else if((button == 0x04) && (scaleCounter == 7)) {
				state = B;
			}
			else if((button == 0x04) && (scaleCounter == 8)) {
				state = C2;
			}
			break;
		case buttonWait:
